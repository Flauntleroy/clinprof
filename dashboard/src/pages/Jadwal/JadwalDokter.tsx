...
export default function JadwalDokter() {
    ...
    const fetchData = async () => {
        ...
        // Fetch doctors
        const dokterRes = await apiGet<{ items: Dokter[] }>("/dokter?active=false&perPage=100");
        if (dokterRes.success && dokterRes.data) {
            setDoctors(dokterRes.data.items);
        }

        // Fetch real jadwal from API
        const jadwalRes = await apiGet<Jadwal[]>("/jadwal?active=false");
        if (jadwalRes.success && jadwalRes.data) {
            // Normalize day names from UPPERCASE to Title Case for UI
            const normalized = jadwalRes.data.map(j => ({
                ...j,
                hari: j.hari.charAt(0) + j.hari.slice(1).toLowerCase(),
                // Format TIME 08:00:00 to 08:00
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload = {
            ...form,
            // Ensure day is uppercase for DB
            hari: form.hari.toUpperCase()
        };

        if (editId) {
            const response = await apiPut(`/jadwal/${editId}`, payload);
            if (response.success) {
