...
import { apiGet, apiPost, apiPut, apiUpload } from "@/lib/dashboard/api";
...
export default function LayananForm({ isEdit = false }: Props) {
    ...
    useEffect(() => {
        if (isEdit && id) {
            const fetchData = async () => {
                setLoading(true);
                const response = await apiGet<LayananForm>(`/layanan/${id}`);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const response = isEdit
            ? await apiPut(`/layanan/${id}`, form)
            : await apiPost("/layanan", form);

        if (response.success) {
            navigate("/layanan");
        } else {
            setError(response.error || "Gagal menyimpan layanan");
        }
        setSaving(false);
    };
    ...
    return (
        <>
            <PageMeta title={`${isEdit ? "Edit" : "Tambah"} Layanan | Makula Bahalap`} />
            <PageBreadcrumb pageTitle={`${isEdit ? "Edit" : "Tambah"} Layanan`} />
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <label className="cursor-pointer inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                                {uploading ? "Mengupload..." : "Upload Ikon"}
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="Contoh: Pemeriksaan Mata Lengkap"
                            />
                        </div>
