...
export default function PengaturanWhatsApp() {
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            setLoading(true);
            const response = await apiGet<{ items: { key: string; value: string }[] }>("/konten");
    ...
    const handleTestMessage = async () => {
        if (!settings.wa_admin) {
            setError("Masukkan nomor admin WhatsApp terlebih dahulu");
            return;
        }
        ...
        try {
            ...
            setError("Gagal mengirim pesan test");
        }
        setTesting(false);
    };
